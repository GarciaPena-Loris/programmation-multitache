...
void * avion (void * p) {
  ...
  pthread_cond_broadcast(&varPartagee->condEmbarquement);
...
void * passager (void * p) {
  ...
  */
  ...
  if (varPartagees->embarquementEnCours == 0) {
    afficher('p', "je patiente en salle d'embarquement", args -> idPassager, args -> destination, args ->couleurTrace);
    pthread_cond_wait(&varPartagees->condEmbarquement, &varPartagees->verrou);
  }
...
int main(int argc, char * argv[]){
  ...
  if (( pthread_cond_init(&varP.condEmbarquement, NULL) != 0)) {
