...
void traitementFTP(int socketClient, char* nomFichier) {
  ...
  while(nbOctetTotalLu < tailleFichier){
    ...
    printf("\tEnvoi n°%lu, nombre d'octets envoyés : %zd, nombre total lus : %zd\n", nbPartie, resSendTCP, nbOctetTotalLu);
...
void traitementFTPinverse(int socketClient, char* nomFichier) {
  ...
  while (nbOctetTotalEcrit < tailleFichier) {
    ...
    printf("\tReception n°%lu, nombre d'octets recus : %ld, nombre total d'octets écrits : %zd\n",nbPartie, resRcvpartie, nbTotalOctetRecu);
