...
void afficherTailleBufferEnvois(int socket) {
    int bufferSize;
    socklen_t taille = sizeof(bufferSize);

    if (getsockopt(socket, SOL_SOCKET, SO_SNDBUF, &bufferSize, &taille) == 0) {
        printf("La taille actuelle du buffer d'envois est : %d\n", bufferSize);
    }
    else {
        perror("\tgetsockopt : ERROR : problème lors de la récupération de la taille du buffer ");
    }
}

void afficherTailleBufferReception(int socket) {
    int bufferSize;
    socklen_t taille = sizeof(bufferSize);

    if (getsockopt(socket, SOL_SOCKET, SO_RCVBUF, &bufferSize, &taille) == 0) {
        printf("La taille actuelle du buffer d'envois est : %d\n", bufferSize);
    }
    else {
        perror("\tgetsockopt : ERROR : problème lors de la récupération de la taille du buffer ");
